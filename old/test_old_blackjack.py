...
def test_ask_number_of_decks_valid(monkeypatch):
    ...
    assert old_blackjack.ask_number_of_decks() == 4


def test_ask_number_of_decks_too_many(monkeypatch):
    """ask_number_of_decks(): a number above the max deck asks the user again."""
    player_chooses([str(old_blackjack.MAX_DECK_PACKS + 1), "2"], monkeypatch)
    assert old_blackjack.ask_number_of_decks() == 2


def test_ask_number_of_decks_max(monkeypatch):
    """ask_number_of_decks(): a number equal to the max is valid."""
    player_chooses([str(old_blackjack.MAX_DECK_PACKS)], monkeypatch)
    assert old_blackjack.ask_number_of_decks() == old_blackjack.MAX_DECK_PACKS
...
def test_ask_number_of_decks_too_few(monkeypatch):
    ...
    assert old_blackjack.ask_number_of_decks() == 3
...
def test_ask_number_of_decks_multiple_attempts(monkeypatch):
    ...
    assert old_blackjack.ask_number_of_decks() == 1
...
def test_ask_number_of_players_single(monkeypatch):
    ...
    assert old_blackjack.ask_number_of_players() == 1
...
def test_ask_number_of_players_valid(monkeypatch):
    ...
    assert old_blackjack.ask_number_of_players() == 3
...
def test_ask_number_of_players_invalid(monkeypatch):
    ...
    assert old_blackjack.ask_number_of_players() == 2


def test_ask_number_of_players_too_many(monkeypatch):
    """ask_number_of_players(): too many players prompts the user to try again."""
    player_chooses([str(old_blackjack.MAX_PLAYERS+1), "2"], monkeypatch)
    assert old_blackjack.ask_number_of_players() == 2


def test_ask_number_of_players_max(monkeypatch):
    """ask_number_of_players(): max players is a valid input"""
    player_chooses([str(old_blackjack.MAX_PLAYERS), "2"], monkeypatch)
    assert old_blackjack.ask_number_of_players() == old_blackjack.MAX_PLAYERS
...
def test_ask_player_name_non_empty(monkeypatch):
    ...
    assert old_blackjack.ask_player_name(1) == "test_name"
...
def test_ask_player_name_empty(monkeypatch):
    ...
    assert old_blackjack.ask_player_name(0) == "Player 1"
...
def test_ask_player_bet_valid(monkeypatch):
    ...
    assert old_blackjack.ask_player_bet(player) == 10.5
...
def test_ask_player_bet_insufficient_funds(monkeypatch):
    ...
    player_chooses([], monkeypatch)
    player = Player("test_name", old_blackjack.MINIMUM_BET - 1.0)
    with pytest.raises(ValueError):
        old_blackjack.ask_player_bet(player)
...
def test_ask_player_bet_equal_to_minimum(monkeypatch):
    ...
    player_chooses([str(old_blackjack.MINIMUM_BET)], monkeypatch)
    player = Player("test_name", 100.0)
    assert old_blackjack.ask_player_bet(player) == old_blackjack.MINIMUM_BET
...
def test_ask_player_bet_greater_than_purse(monkeypatch):
    ...
    player = Player("test_name", 100.0)
    assert old_blackjack.ask_player_bet(player) == 10.0
...
def test_ask_player_bet_multiple_attempts(monkeypatch):
    ...
    player = Player("test_name", 100.0)
    assert old_blackjack.ask_player_bet(player) == 20.0
...
def test_ask_player_action_valid(monkeypatch):
    ...
    assert old_blackjack.ask_player_action(
...
def test_generate_deck_size():
    ...
    assert len(old_blackjack.generate_deck(1)) == 52
    assert len(old_blackjack.generate_deck(2)) == 104
    assert len(old_blackjack.generate_deck(3)) == 156
    assert len(old_blackjack.generate_deck(4)) == 208


def test_generate_deck_cards():
    """generate_deck(): check a single deck contains some expected cards."""
    deck = old_blackjack.generate_deck(1)
    assert ("A", "H") in deck
    assert ("4", "D") in deck
    assert ("J", "H") in deck
    assert ("Q", "C") in deck
...
def test_generate_deck_duplicates():
    ...
    deck = old_blackjack.generate_deck(2)
...
def test_shuffle_returns_copy():
    ...
    deck = old_blackjack.generate_deck(1)
    shuffled_deck = old_blackjack.shuffle(deck, 123456)
...
def test_hand_to_string_base():
    ...
    assert old_blackjack.hand_to_string(hand) == "3H, KD, AH"
...
def test_hand_to_string_empty_hand():
    ...
    hand = Hand()
    with pytest.raises(ValueError):
        old_blackjack.hand_to_string(hand)


def test_hand_to_string_single():
    """hand_to_string(): single card"""
    hand = Hand([("10", "S")])
    assert old_blackjack.hand_to_string(hand) == "10S"
...
def test_define_players_single_player(monkeypatch):
    ...
    players = old_blackjack.define_players(1)
...
def test_define_players_multiple_players(monkeypatch):
    ...
    players = old_blackjack.define_players(3)
...
def test_define_dealer():
    ...
    dealer = old_blackjack.define_dealer()
...
def test_take_bets_base(monkeypatch):
    ...
    players = [
        old_blackjack.Player("test_name1", 100),
        old_blackjack.Player("test_name2", 50),
    ]
    bets = old_blackjack.take_bets(players)
...
def test_take_bets_bet_too_small(monkeypatch):
    ...
    players = [
        old_blackjack.Player("test_name1", 100),
        old_blackjack.Player("test_name2", 200),
    ]
    bets = old_blackjack.take_bets(players)
    assert bets["test_name1"] == 10
...
def test_deal_initial_hands_base():
    ...
    old_blackjack.deal_initial_hands(deck, players, dealer, bets)
...
def test_push():
    ...
    old_blackjack.push(player, hand)
...
def test_lost():
    ...
    player = Player("test_name", 90.)
    hand = Hand([("A", "S"), ("A", "D")], bet=10.)
    old_blackjack.lost(player, hand)
...
def test_one_to_one():
    ...
    player = Player("test_name", 90.)
    hand = Hand([("A", "S"), ("A", "D")], bet=10.)
    old_blackjack.one_to_one(player, hand)
...
def test_three_to_two():
    ...
    player = Player("test_name", 90.)
    hand = Hand([("A", "S"), ("A", "D")], bet=10.)
    old_blackjack.three_to_two(player, hand)
...
def test_make_hand_payment__bust_bust():
    ...
    # Function call
    old_blackjack.make_hand_payment(player, hand, dealer_hand)
    # Assertions
    assert player.purse == 90.
...
def test_make_payments_multiple_players_and_hands():
    ...
    # Function call
    old_blackjack.make_payments(players, dealer)
    # Assertions
...
def player_turn_blackjack():
    ...
    player.give_hand(hand)
    # Function call
    old_blackjack.player_turn(player, deck)
    # Assertions
...
def test_complete_dealer_turn_all_busts():
    ...
    players = [player1, player2]
    # Assertion
    assert old_blackjack.complete_dealer_turn(players, dealer) == False
...
def test_complete_dealer_turn_not_all_busts():
    ...
    players = [player1, player2]
    # Assertion
    assert old_blackjack.complete_dealer_turn(players, dealer) == True
...
def test_dealer_turn_blackjack():
    ...
    # Function call
    old_blackjack.dealer_turn(dealer, deck)
    # Assertions
    assert not hand.is_active
...
def test_reset():
    ...
    # Function call
    old_blackjack.reset([player1, player2], dealer)
    # Assertions
